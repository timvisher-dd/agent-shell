...
=bin/test= runs the full ERT suite in batch mode.  It requires two
environment variables pointing to local checkouts of the dependencies:

#+begin_src bash
  shell_maker_checkout=~/path/to/shell-maker \
  acp_checkout=~/path/to/acp.el \
    bin/test
#+end_src

Both variables are mandatory; the script will exit with a descriptive
