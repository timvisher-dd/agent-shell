...
By default, agent-shell uses the current project root (Projectile or project.el) and falls back to `default-directory`. To override that behavior, customize `agent-shell-cwd-resolver` with either a variable symbol or a function that returns a directory.
...
(setq agent-shell-cwd-resolver (lambda () (expand-file-name "~/work/")))
#+end_src
