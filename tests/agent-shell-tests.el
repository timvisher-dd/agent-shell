...
(ert-deftest agent-shell--resolve-resume-config-prefers-preferred ()
  "Test `agent-shell--resolve-resume-config' prefers preferred config."
  (let ((preferred-config '((:identifier . preferred))))
    (cl-letf (((symbol-function 'agent-shell--resolve-preferred-config)
               (lambda () preferred-config))
              ((symbol-function 'agent-shell-select-config)
               (lambda (&rest _args)
                 (error "Should not prompt for agent selection"))))
      (should (equal (agent-shell--resolve-resume-config) preferred-config)))))

(ert-deftest agent-shell--resolve-resume-config-defaults-to-active-shell ()
  "Test `agent-shell--resolve-resume-config' defaults to active shell config."
  (let ((active-config '((:identifier . active)))
        (captured-default nil))
    (cl-letf (((symbol-function 'agent-shell--resolve-preferred-config)
               (lambda () nil))
              ((symbol-function 'agent-shell--current-shell)
               (lambda () 'active-shell))
              ((symbol-function 'agent-shell--config-for-shell)
               (lambda (_buffer) active-config))
              ((symbol-function 'agent-shell-select-config)
               (lambda (&rest args)
                 (setq captured-default (plist-get args :default-config))
                 active-config)))
      (should (equal (agent-shell--resolve-resume-config) active-config))
      (should (equal captured-default active-config)))))
...
(ert-deftest agent-shell--resume-session-from-list-uses-resume-when-supported ()
  "Test `agent-shell--resume-session-from-list' prefers session/resume."
  (with-temp-buffer
    (let ((requests '())
          (state `((:buffer . ,(current-buffer))
                   (:client . test-client)
      ...
      (should (equal (map-nested-elt agent-shell--state '(:session :id)) "session-123"))
      (should-not (map-elt agent-shell--state :set-model))
      (should-not (map-elt agent-shell--state :set-session-mode))
      (should-not (map-elt agent-shell--state :tool-calls)))))

(ert-deftest agent-shell--resume-session-from-list-falls-back-to-load ()
  "Test `agent-shell--resume-session-from-list' uses session/load when needed."
  (with-temp-buffer
    (let ((requests '())
          (state `((:buffer . ,(current-buffer))
                   (:client . test-client)
                 ...
                 (lambda (&rest args)
                   ...
                   (let* ((request (plist-get args :request))
                     ...
                     (pcase method
                       ("session/list"
                        (funcall (plist-get args :on-success)
                                 '((sessions . [((sessionId . "session-456")
                                                 (cwd . "/tmp")
      ...
      (let ((ordered-requests (nreverse requests)))
        (should (equal (mapcar (lambda (req)
                               ...
                               ordered-requests)
                       '("session/list" "session/load"))))
      (should (equal (map-nested-elt agent-shell--state '(:session :id)) "session-456"))
      (should-not (map-elt agent-shell--state :set-model))
      (should-not (map-elt agent-shell--state :set-session-mode))
      (should-not (map-elt agent-shell--state :tool-calls)))))
