...
(ert-deftest agent-shell--terminal-has-terminal-flag-persists-test ()
  ...
  (let* ((buffer (get-buffer-create " *agent-shell-terminal-has-terminal*"))
         (terminal-id "term_has_terminal")
         (output-buffer nil)
         (agent-shell--state (agent-shell--make-state :buffer buffer)))
    (map-put! agent-shell--state :client 'test-client)
    (map-put! agent-shell--state :request-count 1)
             ...
             :notification `((method . "session/update")
                             (params . ((update . ((sessionUpdate . "tool_call")
                                                    ...
                                                    (content . [((type . "terminal")
                                                                 (terminalId . ,terminal-id))]))))))))
          (with-current-buffer buffer
            (should (map-nested-elt agent-shell--state
                                    '(:tool-calls "call-has-terminal" :has-terminal))))
          (with-current-buffer buffer
            (agent-shell--on-notification
             :state agent-shell--state
             :notification `((method . "session/update")
